...
def generate_maintenance_timeline(date_data):
    ...
    plt.title('Maintenance Activities Timeline')
    plt.tight_layout()
...
def generate_technician_chart(performance_data):
    ...
    plt.bar([i - width/2 for i in x], completed, width, label='Completed Tasks', color='#3498db')
    plt.bar([i + width/2 for i in x], avg_durations, width, label='Avg Duration (hours)', color='#e74c3c')
    
    plt.xlabel('Technicians')
    plt.ylabel('Count / Hours')
    plt.title('Technician Performance')
...
def generate_equipment_manufacturer_chart(manufacturer_data):
    ...
    plt.title('Equipment Distribution by Manufacturer')
    plt.xticks(rotation=45)
    plt.tight_layout()
